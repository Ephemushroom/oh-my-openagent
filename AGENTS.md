...
**Generated:** 2026-09-10 | **Source snapshot:** cb3d88e9c | **Branch:** dev | **Release:** v5.0.0-beta.52 (fork: upstream sync; opencode2 adapter is fork-local)
...
├── packages/                        # Core/MCP/Skills/Adapters/Platform/Web packages. See packages/AGENTS.md
...
│   ├── omo-opencode2/                # Fork-local OpenCode 2 (v2 plugin API) adapter; plan: .omo/plans/opencode2-port.md
│   ├── utils/ model-core/ prompts-core/ rules-engine/ agents-core/ agents-md-core/ comment-checker-core/ hashline-core/ boulder-state/ memory-core/ telemetry-core/ lsp-core/ mcp-stdio-core/ tmux-core/ claude-code-compat-core/ skills-loader-core/ mcp-client-core/ openclaw-core/ team-core/ delegate-core/ omo-config-core/   # shared Core packages
...
- **Runtime:** Bun only (1.4.2, pinned identically in CI and `.devcontainer/Dockerfile`). Never npm/yarn/pnpm. (Exceptions: `packages/lsp-tools-mcp` + `packages/lsp-daemon` are Node-targeted, vendored, and built with `npm` + vitest/biome.)
...
|---------|------------------|------|
| GitHub Codespaces / VS Code Dev Containers | [`.devcontainer/devcontainer.json`](.devcontainer/devcontainer.json) + [`.devcontainer/Dockerfile`](.devcontainer/Dockerfile) (Node 24 + Bun 1.4.2 + tmux, matching CI) | `postCreateCommand` runs `setup.sh` on container create |
...
- A required check that is already red on `dev` is a base-branch defect and remains a merge blocker. Inspect the latest `dev` run, reproduce the failure on the matching platform and toolchain, root-fix it in the current PR or a separate atomic PR, rerun every required check, and record the evidence.
...
- **Agent state directory:** ONE canonical location, `~/.omo/agent`, resolved through `canonicalAgentDir()` in [`packages/omo-native/bin/lib/agent-dir.js`](packages/omo-native/bin/lib/agent-dir.js) and its adapter-side twin `resolveAgentHome()` in `packages/omo-senpi/src/components/agent-home/`. Every OMO entry point must use that helper; explicit `OMO_CODING_AGENT_DIR`, `SENPI_CODING_AGENT_DIR`, or `PI_CODING_AGENT_DIR` overrides still win.
