#!/usr/bin/env bash
# omo-opencode2 ultrawork banner parity QA driver.
# Subject under test: the ultrawork injection now lands INLINE on the user's own
# message (v1 placement), not as a detached system part — so the model treats
# "first response" literally and says ULTRAWORK MODE ENABLED! before thinking.
#
# Proofs:
#  - trace omo.context.composed modeTagged=true now sourced from MESSAGES
#  - exported session: the LAST user message carries <ultrawork-mode> inline
#    (after the --- separator); no system part carries the tag
...
note "step 1: ulw prompt — mode tag injected inline on the user message"
run_case run-ulw "ulw: reply with the single word ok."
trace_has '"event":"omo.context.composed".*"modeTagged":true'; check "ulw.mode.tagged" $? "context composer tagged a message (trace, message-sourced)"

note "step 2: exported session proves the directive reached the model (dispatch-time draft)"
