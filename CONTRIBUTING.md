...
- **Bun** 1.4.2 (CI-pinned) - The only package manager for the workspace itself
...
| Harness | Wiring |
| ------- | ------ |
| GitHub Codespaces / VS Code Dev Containers | `.devcontainer/devcontainer.json` runs `postCreateCommand: script/agent/setup.sh` on `.devcontainer/Dockerfile` (Node 24 + Bun 1.4.2 + tmux) |
