...
| `agents-core/` | `src/`, `test/`, `tsconfig.json` | Fork-local harness-neutral agent prompt builders, routing and metadata shared by the OpenCode and OpenCode2 adapters. |
...
- **`omo-opencode2`** is the OpenCode 2 adapter (v2 Promise plugin API, `@opencode/plugin`). It ports the highest-value v1 surface onto v2's imperative registration API: agents, hashline edit/read pairing, tool guards, comment checker, rules injection, delegation, shared skills, executable commands, and gated continuation features. Source-checkout installs configure the `packages/omo-opencode2/src` directory; the package manifest pins the supported SDK version. Port design: [.omo/plans/opencode2-port.md](../.omo/plans/opencode2-port.md); QA regimen: [.agents/skills/opencode2-qa/](../.agents/skills/opencode2-qa/). See [packages/omo-opencode2/AGENTS.md](omo-opencode2/AGENTS.md).
...
- **`packages/web/` is excluded from root `bun test`** via `bunfig.toml`. On this fork it has no separate CI (the fork slimmed its CI to opencode2-only).
