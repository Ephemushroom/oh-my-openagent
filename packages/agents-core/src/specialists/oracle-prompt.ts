...
export const ORACLE_AGENT_DESCRIPTION =
...
export interface OraclePromptSelection {
  prompt: string;
...
/**
 * Selects the model-family Oracle prompt variant plus the GPT-only reasoning/
...
export function getOraclePromptSelection(model: string): OraclePromptSelection {
  ...
  if (isGpt5_5Model(model)) {
    return { prompt: ORACLE_GPT_5_5_PROMPT, reasoningEffort: "medium", textVerbosity: "high" };
  }
  if (isGptModel(model)) {
    return { prompt: ORACLE_GPT_PROMPT, reasoningEffort: "medium", textVerbosity: "high" };
  }
  return { prompt: ORACLE_DEFAULT_PROMPT };
}
