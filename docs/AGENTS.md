...
Tracked Markdown files across 6 subdirectories (guide, reference, examples, legal, templates, troubleshooting) plus root files. Categorized by audience: user-facing guides and reference, troubleshooting, legal. The website at [packages/web/](../packages/web) consumes some of these.
...
- **Docs touching `packages/web/`** do not re-trigger web CI on this fork (the fork slimmed its CI to opencode2-only).
